...
const CONFIG = {
  ...
  mapsUrl: 'https://maps.app.goo.gl/K8u5jL7xN7Q6p1Xy9',
  hours: '08:30 – 19:00',
  businessName: 'NeTT',
  logoPath: 'logo.png',
...
function renderStickyCTA() {
  ...
  actions.innerHTML = `
    ...
    <a href="${CONFIG.phoneTel}" class="floating-btn phone" data-label="Hemen Araç" aria-label="Bizi Araç">${ICONS.phone}</a>
  `;
  document.body.appendChild(actions);
  const mobileBar = document.createElement('div');
  mobileBar.className = 'mobile-call-bar';
  mobileBar.innerHTML = `
    <a href="${CONFIG.phoneTel}" class="btn btn-primary" style="flex:1">${ICONS.phone} Hemen Araç</a>
...
function initPage(opts = {}) {
  ...
  if (!opts.skipSchema) {
    const bŞaseSchema = {
      ...
      "sameAs": ["https://maps.google.com/?q=NeTT+Oto+Boya"]
    };
    if (opts.schema) {
      renderSchema(Object.assign({}, bŞaseSchema, opts.schema));
    } else {
      renderSchema(bŞaseSchema);
    }
  }
